...
/// Returns true when styled/colored output should be used.
/// Respects the `CLICOLOR_FORCE` env var (set by `--slow` mode).
...
fn extract_codex_project_from_file(path: &Path) -> Option<String> {
...
fn extract_session_title(source: SourceKind, path: &Path) -> String {
...
fn is_codex_system_content(s: &str) -> bool {
    let t = s.trim();
